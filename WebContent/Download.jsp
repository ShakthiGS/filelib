...
PreparedStatement p = con.prepareStatement("SELECT * FROM file.manageract where sfemail='"+email+"' and status='sended'");
...
<td><a href="staffdownloadjava.jsp?id=<%=rs.getString(1) %>&&fname1=<%=rs.getString(3)%>&&fkeyl=<%=rs.getString(6)%>&&headkey=<%=file %>"><button class="btn-btn-info btn-sm"> Download</button></a></td>
</tr>
<%}%>
            </table>
        </center>
